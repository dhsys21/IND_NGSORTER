...
void __fastcall TAlarmForm_fms::ShowFmsError(const AnsiString &Title,
	...
	int ResponseValue, bool PauseMotion)
{
	// Keep the popup safe even when a future caller bypasses ShowFmsAlarm().
	// Only the reference Scan caller opts out. Production/remote alarms still pause.
	if(PauseMotion && gripper != NULL) gripper->req_Pause(true);
	if(PauseMotion && robostar != NULL) robostar->req_Pause(true);
	operatorPaused = !PauseMotion; // A closed reference error does not reopen itself.
	dismissed = false;
	...
	lblStatus->Caption = PauseMotion ?
		"Auto-paused. Pause: hold alarm / Close: hide only. Correct FMS, then press Retry or Main Restart." :
		"Reference lookup only. Retry: read again / Close: hide / AUTO or Init Work: discard lookup. Motion unchanged.";
	btnPause->Enabled = PauseMotion;
